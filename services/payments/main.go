...
func main() {
	...
	// #region agent log
	agentLog("H1", "payments/main.go:33", "waitForDB.start", map[string]any{"db": cfg.DBURL})
	// #endregion
	if err := waitForDB(ctx, dbConn, 30*time.Second); err != nil {
		// #region agent log
		agentLog("H1", "payments/main.go:36", "waitForDB.fail", map[string]any{"err": err.Error()})
		// #endregion
		log.Fatalf("db not ready: %v", err)
	}
	// #region agent log
	agentLog("H1", "payments/main.go:41", "waitForDB.ok", map[string]any{})
	// #endregion
	...
	if err := declareQueues(ch); err != nil {
		// #region agent log
		agentLog("H3", "payments/main.go:65", "declareQueues.fail", map[string]any{"err": err.Error()})
		// #endregion
		log.Fatalf("declare queues: %v", err)
	}
	// #region agent log
	agentLog("H3", "payments/main.go:70", "declareQueues.ok", map[string]any{})
	// #endregion
...
func waitForRabbit(ctx context.Context, url string, timeout time.Duration) (*amqp.Connection, error) {
	...
	for {
		// #region agent log
		agentLog("H2", "payments/main.go:90", "rabbit.dial.try", map[string]any{"url": url})
		// #endregion
		conn, err := amqp.Dial(url)
		if err == nil {
			// #region agent log
			agentLog("H2", "payments/main.go:94", "rabbit.dial.ok", map[string]any{})
			// #endregion
			return conn, nil
		}
		if time.Now().After(deadline) {
			// #region agent log
			agentLog("H2", "payments/main.go:99", "rabbit.dial.timeout", map[string]any{"err": err.Error()})
			// #endregion
			return nil, err
		}
		time.Sleep(time.Second)
